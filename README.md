...
We can choose to receive messages inside the group, or inside a personal message. We can remove the repository just from the message list. When we reply to the bot's message, our feedback is sent to GitHub via the bot.
Detailed explanation of each option and how to work with the Telegram bot.
First, we open the bot's link and start the bot.
To connect your GitHub account to the Telegram GitHub bot, we click on "connect".
After clicking, we need to open a link. If Telegram is filtered in your country, you should use a VPN.
After you have connected the Telegram bot to your GitHub account, you can use it.
Note that this bot only sends you messages related to your own repository, repositories you have forked, or repositories where someone has sent you a pull request. And when someone has sent you a request, you can reply to their comments or even to their request through the Telegram bot.
Options available in the Telegram bot commands:
/connect Authorize GitHub bot for permissions via OAuth: With this option, the GitHub account is connected to the Telegram bot.
/newintegration Add new GitHub repository integration: We can add a repository to our Telegram bot. When we click on this option, it shows us two ways to receive messages: through personal chat, or through the group.
/listintegrations List all current integrations: The list of repositories can be viewed this way.
/delintegration Delete integration: This removes repositories from the bot's repository list, but the repositories are not deleted from the GitHub account.
---
...
توضیح دقیق هر گزینه و روش کار با ربات تلگرام.
اول، لینک ربات رو باز میکنیم و ربات رو استارت میکنیم.
برای اینکه حساب گیتهاب رو به ربات تلگرامی گیتهاب وصل کنیم، روی conect کلیک میکنیم.
بعد از کلیک، باید یک لینک رو باز کنیم، باید اگر تلگرام در کشورتون فیلتر هست، از فیلترشکن استفاده کنید.
بعد از اینکه ربات تلگرام رو به حساب گیتهاب وصل کردید، میتونید ازش استفاده کنید.
توجه داشته باشید که این ربات فقط پیامهایی که مربوط به ریپازیتوری خودتون هست، یا ریپازیتوریهایی که فورک (fork) کردید، یا ریپازیتوریهایی که کسی براتون درخواست کشش (pull request) فرستاده رو براتون میفرسته، و وقتی که درخواست فرستاده باشه براتون یه نفر، میتونید به کامنتهاش یا حتی روی درخواستش از طریق ربات تلگرام پاسخ بزنید.
گزینه های موجود در دستورات ربات تلگرام:
/connect Authorize GitHub bot for permissions via OAuth: با کمک این گزینه، حساب گیتهاب به ربات تلگرام وصل میشه
/newintegration Add new GitHub repository integration: میتونیم به ربات تلگراممون ریپازیتوری اضافه کنیم، وقتی روی این گزینه کلیک کنیم، بهمون دو راه دریافت پیام نشون میده، از طریق چت شخصی، یا از طریق گروه
/listintegrations List all current integrations: فهرست ریپازیتوریها رو میشه به این طریق دید.
/delintegration Delete integration: از این طریق ریپازیتوریها رو از لیست ریپازیتوریهای ربات حذف کرد، ولی ریپازیتوریها از حساب گیتهاب حذف نمیشه.
---
